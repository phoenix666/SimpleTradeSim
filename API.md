...
- `shortFloat(n)` - Форматирует число в читаемый вид: числа < 1000 показываются с 2 знаками после запятой, >= 1000 показываются с суффиксом K (тысячи) или M (миллионы). Используется для отображения баланса, позиции, PnL. Возвращает строку.
...
- `increasePosition(marginSize, positionSize)` - Увеличивает позицию: вычитает маржу из cashBalance (не более 80% от баланса), добавляет amount к currentPosition, обновляет positionMargin и positionCostBasis. При открытии новой позиции записывает цену входа в `entryPrice`. Логирует операцию.
...
- `logOperation(action, amount, pnl)` - Записывает операцию в лог. `action` - тип операции (Long/Short/Close/Partial close), `amount` - количество актива (форматируется через shortFloat), `pnl` - финансовый результат (0 если позиция только открыта, форматируется через shortFloat). PnL окрашивается: прибыль - зеленый, убыток - красный.
...
- `adjustPositionSize(direction)` - Изменяет positionSize в 2 раза. `direction` > 0 увеличивает, < 0 уменьшает. При уменьшении до 0.5 переходит к 0.5. Значение отображается через shortFloat().
...
- `input[name="candleCount"].change` - Изменяет количество видимых свечей на графике (CANDLE_COUNT) и перерисовывает график.
...
- `CANDLE_COUNT` - Количество видимых свечей на графике (по умолчанию 20, может быть изменено через радио-кнопки: 5, 10, 20, 40, 60).
- `shortFloat(n)` - Форматирует число в читаемый вид: числа < 1000 показываются с 2 знаками после запятой, >= 1000 показываются с суффиксом K (тысячи) или M (миллионы). Используется для отображения баланса, позиции, PnL. Возвращает строку.
...
- `#currentPositionDisplay` - Отображает текущую позицию (Position).
- `#unrealizedGainDisplay` - Отображает нереализованную прибыль/убыток (Gain/Loss).
- Радио-кнопки `input[name="candleCount"]` - Выбор количества видимых свечей на графике (5, 10, 20, 40, 60). Расположены поверх графика.
