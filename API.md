...
- Обработчики событий:
  ...
  - `randomBtn.click` - Устанавливает случайный lastIndex, сбрасывает торговые переменные в начальные значения и очищает stopLoss/profitLimit.
  - `stepForwardBtn.click` - Увеличивает lastIndex на 1, пересчитывает позицию пропорционально изменению цены. При наличии стопа/лимита проверяет их срабатывание через checkStopLimitClose.
